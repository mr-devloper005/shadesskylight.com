...
const taskGrid: Record<TaskKey, string> = {
  article: 'grid gap-5 sm:grid-cols-2 lg:grid-cols-3',
  listing: 'grid gap-4 sm:grid-cols-2',
...
const cardBase = 'group block rounded-xl border border-[var(--editable-border)] bg-white transition duration-300 hover:-translate-y-1 hover:shadow-md'
...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  return (
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--editable-page-bg)] text-[var(--slot4-page-text)]">
        <header className="relative overflow-hidden border-b border-[var(--editable-border)] bg-[#eef4f2]">
          <div className="relative mx-auto max-w-[var(--editable-container)] px-4 py-8 sm:px-6 lg:px-8">
            <div className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.18em] text-[#185d63]">
              <span>{theme.kicker}</span>
              <span className="h-1 w-1 rounded-full bg-[#185d63] opacity-50" />
              <span className="text-[#516176]">{label}</span>
            </div>
            <h1 className="editable-display mt-4 max-w-3xl text-balance text-3xl font-black leading-tight tracking-normal sm:text-4xl">
              {voice?.headline || `Browse ${label}`}
            </h1>
            <p className="mt-3 max-w-2xl text-base leading-7 text-[#516176]">{voice?.description || theme.note}</p>
            {voice?.chips?.length ? (
              <div className="mt-8 flex flex-wrap gap-2.5">
                {voice.chips.map((chip) => (
                  <span key={chip} className="rounded-full border border-[var(--editable-border)] bg-white px-3.5 py-1.5 text-xs font-bold text-[#516176]">{chip}</span>
                ))}
              </div>
            ) : null}

            <div className="mt-7 flex flex-col gap-4 rounded-md border border-[var(--editable-border)] bg-white p-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-[#516176]">
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-11 appearance-none rounded-md border border-[var(--editable-border)] bg-white pl-4 pr-10 text-sm font-bold text-[#071a2d] outline-none transition focus:border-[#185d63]"
                    aria-label={voice?.filterLabel || 'Filter category'}
                  >
                    <option value="all">All categories</option>
                    {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
                  </select>
                  <ChevronDown className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#516176]" />
                </div>
                <button className="inline-flex h-11 items-center rounded-md bg-[#185d63] px-5 text-sm font-bold text-white transition hover:opacity-90">Apply</button>
              </form>
            </div>
          </div>
        </header>
        ...
        <div className="mx-auto max-w-6xl px-4 py-6">
          <Ads slot={task === 'listing' ? 'sidebar' : 'header'} showLabel eager className="mx-auto w-full" />
        </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-8 sm:px-6 lg:px-8">
          {posts.length ? (
            <>
              ...
              <div className="mx-auto max-w-6xl px-4 py-6">
                <Ads slot={task === 'listing' ? 'footer' : 'in-feed'} showLabel className="mx-auto w-full" />
              </div>
            </>
          ) : (
            <div className="mx-auto max-w-xl rounded-xl border border-dashed border-[var(--editable-border)] bg-white px-8 py-16 text-center">
              <Search className="mx-auto h-7 w-7 text-[#516176]" />
              <h2 className="editable-display mt-5 text-2xl font-semibold tracking-[-0.02em]">Nothing here yet</h2>
              <p className="mt-2 text-sm leading-6 text-[#516176]">Try another category, or check back after new {label.toLowerCase()} are published.</p>
            </div>
          )}
  ...
  return (
    <Link href={href} className={`${cardBase} flex h-full flex-col overflow-hidden`}>
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        <div className="flex items-center gap-2 text-[10px] font-medium uppercase tracking-[0.2em] text-[var(--tk-accent)]">
          <span className="truncate">{category}</span>
          <span className="text-[var(--tk-muted)]">· {String(index + 1).padStart(2, '0')}</span>
        </div>
        <h2 className="editable-display mt-2 line-clamp-2 text-[17px] font-semibold leading-snug tracking-[-0.01em]">{post.title}</h2>
        <RatingLine post={post} />
        <p className="mt-2 line-clamp-2 text-[13px] leading-6 text-[var(--tk-muted)]">{getSummary(post)}</p>
        <span className="mt-auto inline-flex items-center gap-1.5 pt-3 text-xs font-semibold text-[var(--tk-accent)]">
          Read article <ArrowUpRight className="h-3.5 w-3.5 transition duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
        </span>
      </div>
    </Link>
  )
